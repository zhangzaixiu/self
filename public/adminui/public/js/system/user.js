    ...
    var $userAccont = $("#userAccountInfo");
     ...
     App.extend({
        ...
        handleEdit: function () {
            ...
            $(document).on('click', '.btn-edit, #addRoleToggle', function (e) { // 编辑角色信息
                ...
                $('[data-plugin="treeview"]').data('jstree', false).empty().jstree({
                    ...
                    "core": {
                        'data': {
                            "url": $.ctx + '/role/menuList?roleId=' + (ID === undefined ? -1 : ID),
                            "dataType": "JSON"
                        }
                    }
                });
            ...
            $('#roleForm').on('hide.bs.modal', function () { // 添加角色模态窗 在hide方法调用时（但还未关闭隐藏）立即触发
                $('#roleInfo').formValidation('resetForm', true);//清除历史错误提示信息
            });
        },
        ...
        roleInfoCallback: function (validator, data) {
            ...
            $('#roleForm').one('hidden.bs.modal', function () {//one事件只会触发一次  在模态框隐藏之后
